...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        average_train_accuracy = sum(metrics["train_acc"]) / len(metrics["train_acc"])
        Logger.add_scalar("train_accuracy", average_train_accuracy, global_step)
        ...
        average_val_accuracy = sum(metrics["val_acc"]) / len(metrics["val_acc"])
        Logger.add_scalar("val_accuracy", average_val_accuracy, global_step)
